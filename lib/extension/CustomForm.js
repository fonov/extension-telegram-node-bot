...
class runCustomForm extends global.Telegram.BaseScopeExtension {
    process(formData, callback) {
        ...
        const run = () => {
            ...
            if (i == keys.length) {
                try {
                    if(formData.config.confirm.skip)
            ...
            for(var y = 0; i+1 > y; y++){
                if(y == i)
                    message += `\n<i>${key.text}${(key.question) ? '?' : ''}</i>${key.extramess}`
                else {
                    message += `\n<i>${formData.form[keys[y]].text}</i>`
                    if(typeof result[keys[y]] === 'string' || typeof result[keys[y]] === "number")
